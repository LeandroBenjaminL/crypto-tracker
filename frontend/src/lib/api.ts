...
export async function getPrices(queries: string[]): Promise<CoinResult[]> {
	...
	try {
		return await _fetch<CoinResult[]>(`${API_BASE}/api/prices?q=${q}`, 6000);
	} catch {
		// Fallback individual
		const results = await Promise.allSettled(queries.map((s) => getPrice(s)));
		return results
...
export async function getTop(limit = 10): Promise<CoinResult[]> {
	return _fetch<CoinResult[]>(`${API_BASE}/api/top?limit=${limit}`, 8000, () =>
		_cgTop(limit),
	);
}
