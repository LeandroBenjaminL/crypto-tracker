...
const SYMBOL_TO_ID: Record<string, string> = {
  ...
  ltc: "litecoin", bch: "bitcoin-cash", trx: "tron", xlm: "stellar",
  etc: "ethereum-classic", fil: "filecoin", apt: "aptos", sui: "sui",
  near: "near", icp: "internet-computer", vet: "vechain", aave: "aave",
  grt: "the-graph", sand: "the-sandbox", mana: "decentraland", axs: "axie-infinity",
  shib: "shiba-inu", pepe: "pepe", bonk: "bonk", wif: "dogwifhat",
  floki: "floki", dai: "dai", usdc: "usd-coin", usdt: "tether",
  cro: "crypto-com-chain", ton: "the-open-network", hbar: "hedera-hashgraph",
  algo: "algorand", inj: "injective-protocol", rune: "thorchain",
  pendle: "pendle", fet: "fetch-ai", tao: "bittensor", render: "render-token",
  strk: "starknet", sei: "sei-network", dydx: "dydx-chain", ena: "ethena",
  ondo: "ondo-finance", jup: "jupiter-exchange-solana", jto: "jito-governance-token",
};
...
async function _cgPrice(symbol: string): Promise<CoinResult | null> {
  ...
  try {
    const data = await _fetch<Record<string, { usd: number; usd_24h_change?: number }>>(
    ...
    if (!coin) return null;
    return {
      id, symbol, name: id.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase()),
      rank: 0, price: coin.usd, change_24h: coin.usd_24h_change ?? null,
      volume_24h: null, market_cap: null, price_formatted: fmtPrice(coin.usd),
    };
  } catch {
    return null;
  }
...
async function _cgTop(limit = 10): Promise<CoinResult[]> {
  try {
    ...
    return (list || []).map((c: any) => ({
      id: c.id, symbol: c.symbol, name: c.name, rank: c.market_cap_rank ?? 0,
      price: c.current_price, change_24h: c.price_change_percentage_24h,
      volume_24h: c.total_volume, market_cap: c.market_cap,
...
async function _cgSearch(query: string): Promise<CoinResult[]> {
  try {
    ...
    return (data.coins || []).slice(0, 20).map((c: any) => ({
      id: c.id, symbol: c.symbol, name: c.name, rank: c.market_cap_rank ?? 0,
      price: null, change_24h: null, volume_24h: null, market_cap: null, price_formatted: null,
    }));
  } catch {
    return [];
  }
